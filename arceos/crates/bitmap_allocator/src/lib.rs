...
pub trait BitAlloc: Default {
    ...
    /// Allocate specific bit
    fn alloc_fixed_bit(&mut self, key: usize) -> Option<usize>;
...
impl<T: BitAlloc> BitAlloc for BitAllocCascade16<T> {
    ...
    fn alloc_fixed_bit(&mut self, key: usize) -> Option<usize> {
        let i = key / T::CAP;
        if key < Self::CAP && self.sub[i].alloc_fixed_bit(key % T::CAP).is_some() {
            self.bitset.set_bit(i, !self.sub[i].is_empty());
            Some(key)
        } else {
            None
...
impl BitAlloc for BitAlloc16 {
    ...
    fn alloc_fixed_bit(&mut self, key: usize) -> Option<usize> {
        if key < Self::CAP && self.0.get_bit(key) {
            self.0.set_bit(key, false);
            Some(key)
        } else {
            None
        }
